...
- AC-14b: 目標 `devflow/` 下任一 symlink（不論指向何處、是否 dangling）的相對路徑是鏡像集合某路徑的祖先（例：來源有 `devflow/sub/a`，目標 `devflow/sub` 是 symlink）→ 決策階段 exit 2，stderr `devflow/sub: symlink where source has directory`；不寫任何檔。祖先關係以路徑字串判，不 resolve；非祖先的 symlink 依「動作」定義為 `updated`（來源同路徑是檔）或 `deleted`（來源無）
...
- AC-16: 寫入順序＝鏡像 created／updated（每檔單檔原子）→ 鏡像 deleted → 空目錄移除 → `devflow.local/` → 入口檔（入口規格原子性）。寫入階段 I/O 失敗 → **exit 3**，stderr 恰一行 `<失敗路徑>: <OSError strerror>`；已完成的步驟不回滾（可觀察判準：失敗路徑之前的動作已落盤、之後的未動），重跑即為恢復（AC-5 保證收斂）。harness 以 `updated` 兩檔、第二檔所在目錄於決策後改為不可寫（或以 `monkeypatch os.replace` 對第二檔拋 OSError）構造
