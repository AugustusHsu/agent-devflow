...
# 十七項裡十二項是 True（`encoding`、`d2`、`i1`、`i5`、`version`、`fence`、`tables`、`table`、
# `link`、`r9`、`dupid`、`v7`），五項是 False（`refs`、`seatoblig`、`orphan`、`r2`、`r9sub`）
# ——分界不是「哪一項比較重要」，而是**定義域封不封閉**，見下面各節。
# 五項建議停在建議的理由各不相同：`refs` 是定義域封不住（見下面「擋不住什麼」）；
# `seatoblig`／`orphan` 的定義域封閉，只是正反測試還沒補（issue #213；升關卡的前提同樣是
# 上面那條 README 的判定方式）；`r9sub` 的定義域同樣封閉（和 `r9` 同一個定義域，只多問
# `📝` 格有沒有標子類），正反測試在同一個 PR 裡補齊（issue #190），升關卡的前提仍是上面
# 那條 README 的判定方式要的 CI 正反兩個 run；`r2` 的定義域也封閉，但它**依裁決永遠不升關卡**
...
# 十七項裡十二項是 True，五項是 False。分界是定義域封不封閉，理由見檔頭。
...
GATES = {
    ...
    "r9sub":   False,   # R9 的 `📝` 格須標明是哪一個子類（issue #190；定義域同 `r9`、封閉，
                        # 正反測試在同一個 PR，升 required 的前提是上面那條 README 的判定方式）
...
R9_SEPS = " \t（(：:，,。、；;）)"
# `📝 已宣稱` 的兩個子類名，用詞以 devflow/WORKFLOW.md 的 R9 為準（「依該格有無第三者現在
...
print()
print("── R9 子類：📝 格須標明 R9 的哪一個子類（%s）" % tag("r9sub"))
# `R9` 對 `📝 已宣稱` 比三值多要一件事：「依該格有無第三者現在能執行的驗證方式分兩種，
# 須標明是哪一種」。上面那一節只判狀態格取不取三值之一，`📝` 有沒有標子類它一律放過——
# 本節補這一項（issue #190）。
#   定義域：和 `r9` **同一個**、同樣封閉——`table` 那一節判定為「合格」的表（status_tables）
#       的資料列狀態格，且只看以 `📝` 起頭的那些格。欄位一律走 status_tables 帶著的
#       狀態欄索引（tables_of() 的 token 解析），**不自己切 `|`**：對照表的格子裡有跳脫的
#       `\|`（值欄與狀態欄都有），按字面切會切在跳脫的那個 `|` 上，取到的「狀態欄」是
#       別的欄位的碎片——該報的漏報、位置也錯（issue #190 的反測 `r9sub:escaped-pipe`）。
#       **表格以外的敘述句不掃**：同 `r9` 的理由——討論子類用詞沿革、寫給讀者的遷移說明
#       都會提到子類名，掃散文是 required gate 的假陽性（審查者 PR #95 反例）。
#   判準：子類名**出現在該格裡**，不要求特定寫法。現行九格有兩種寫法——
#       `📝 已宣稱（`R9` 子類「驗證未達 `✅`」：…）` 與 `📝 已宣稱（驗證未達 `✅`：…）`
#       （devflow/coders/claude-code.md）——`R9` 只要求「標明是哪一種」，把它收成單一寫法
#       就是檢查器發明規則（見檔頭「不發明規則」）。
#   仍擋不住：標的那一種**對不對**（該格到底有沒有第三者現在能執行的驗證方式）、一格兼有
#       兩者時有沒有依 `R7` 逐項標明。那是語意，檢查器判不了，仍靠 `R4` 人工審查（母單 #22）。
#       本節只鎖住「兩種之一有被寫出來」。
r9sub_bad = 0
r9sub_cells = 0
for f in table_files:
    for t, col in status_tables.get(f) or []:
        # 面向欄一定在表頭（`table` 那一節要求表頭含 TABLE_HEADER 全部三個），但資料列的
        # 格數只保證**大於狀態欄索引**（見該節第 (5) 條：parser 補格不是可依賴的契約），
        # 所以取面向格仍要防越界。
        acol = t["header"].index(TABLE_HEADER[0])
        for r in t["body"]:
            cell = r["cells"][col]
            if not cell.startswith("📝"):
                continue
            r9sub_cells += 1
            if any(sub in cell for sub in R9_SUBS):
                continue
            r9sub_bad += 1
            report("r9sub",
                   "%s:%s 的 📝 格沒有標明 R9 的子類（面向「%s」）"
                   % (f, r["line"],
                      r["cells"][acol] if acol < len(r["cells"]) else "（缺格）"),
                   ["狀態欄=「%s」" % cell,
                    "R9 的兩種：%s" % "／".join("「%s」" % s for s in R9_SUBS)])
if not r9sub_bad:
    ok("%d 個 📝 格都標明了 R9 的子類（%s）"
       % (r9sub_cells, "／".join("「%s」" % s for s in R9_SUBS))
       + ("" if GATES["r9sub"] else
          " —— 可把 GATES[\"r9sub\"] 改成 True 升為必需關卡"
          "（前提見 README 第三出口的判定方式）"))
