...
#   定義域：兩件事，都封閉——
#       (1) `table` 那一節判定為「合格」的表（status_tables）的資料列狀態格。形狀不合的
#           表不進來：它們已經被 `table` 擋，這裡不重複報，也不在沒有格式契約的東西上判值。
#       (2) 這七個檔裡**表格以外**的行是否仍用舊的二值用語（OLD_TOKENS）——敘述句把舊制度
#           寫回去，等於文件自己宣告另一套狀態值。
#       **值欄該記什麼不在定義域內**：本項只判狀態格取不取三值之一，不判補充寫得對不對。
#       那個未定案（#13／#22）因此不再是本項升關卡的前置——它擋的是別的東西。
...
# 「r9 為什麼可以是關卡」。`✅` 的驗證方式與受測環境該記成什麼仍未定案（issue #13、#22），
# 但那是補充**內容**的問題，不在本項的定義域內——本項只判狀態格取不取三值之一。
