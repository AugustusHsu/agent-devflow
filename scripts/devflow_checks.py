...
#     TEXT_TOKENS 那一小組，其餘型別一律不看；這些規則的行內語法產生的都是「自己的開／關
#     token（不是文字）＋自己那一層的內文」，內文算不算裸文字由前綴判準決定，不必逐一補。
...
#     裝飾放行。啟用新外掛＝要重新看它產出什麼 token，不能靠這一段推論。（外掛這一項依官方
#     原始碼核對，未實跑：本機沒有 mdit_py_plugins。）
#   新的誤認風險（誠實記下，不宣稱封閉）：
...
def container_stacks(children):
    ...
    **不帶容器身分**。容器關閉後深度會被重用，於是「已關閉的 sibling 容器內文」與
    ...
    不成對的 `*_close`（parser 正常不會產生）彈到空堆疊為止，不拋例外——判準不因
    畸形輸入而變成 exit 2。
    """
    out = []
    stack = []
