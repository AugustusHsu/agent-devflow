...
NEGATIVE = [
    ...
    ("步驟 4 的區塊整段縮排四格（縮排 ≥4 格不是 fence）",
     ...
     {"blocks"}),
    # ── 前置選項的引號值（issue #189 目標 1；#188 R1 BLOCK 1 前半） ──
    ('步驟 2 補一行 git -C "a b" push -f（-C 的雙引號值含空白）',
     lambda t: append_to_block(t, 2, 'git -C "a b" push -f origin refs/tags/x'),
     {"d"}),
    ("步驟 2 補一行 git -C 'a b' push -f（-C 的單引號值含空白）",
     lambda t: append_to_block(t, 2, "git -C 'a b' push -f origin refs/tags/x"),
     {"d"}),
    ('步驟 2 補一行 git -c user.name="a b" push -f（-c 的值一段加引號）',
     lambda t: append_to_block(t, 2, 'git -c user.name="a b" push -f origin refs/tags/x'),
     {"d"}),
    ('步驟 2 補一行 git --work-tree "a b" push --delete（帶值長選項，空白接引號值）',
     lambda t: append_to_block(t, 2, 'git --work-tree "a b" push --delete origin "refs/tags/$tag"'),
     {"d"}),
    ('步驟 2 補一行 git --git-dir="a b" tag -d（帶值長選項，= 接引號值）',
     lambda t: append_to_block(t, 2, 'git --git-dir="a b" tag -d "$tag"'),
     {"d"}),
    ('步驟 2 補一行 git --exec-path="/a b" push -f（其餘長選項，= 接引號值）',
     lambda t: append_to_block(t, 2, 'git --exec-path="/a b" push -f origin refs/tags/x'),
     {"d"}),
    ('步驟 2 補一行 git -C \\"a push -f b\\"（`\\"` 讓引號配對與 shell 不一致；`\\S+` 那一趟照樣接得住）',
     lambda t: append_to_block(t, 2, 'git -C \\"a push -f b\\"'),
     {"d"}),
    # ── 展開正規化（issue #189 目標 2；#188 R1 BLOCK 1 後半、BLOCK 2） ──
    ("步驟 2 補一行 git${IFS}push${IFS}-f（${IFS} 分詞）",
     lambda t: append_to_block(t, 2, "git${IFS}push${IFS}-f${IFS}origin${IFS}refs/tags/x"),
     {"d"}),
    ("步驟 2 補 git fetch --upload-pack='git p${x}ush …'（空變數拆字）",
     lambda t: append_to_block(
         t, 2, "git fetch --upload-pack='git p${x}ush origin +refs/heads/x:refs/heads/x' ."),
     {"d"}),
    ('步驟 2 補一行 git p""ush -f（空雙引號拆字）',
     lambda t: append_to_block(t, 2, 'git p""ush -f origin "refs/tags/$tag"'),
     {"d"}),
    ("步驟 2 補一行 git tag -''d（空單引號拆旗標）",
     lambda t: append_to_block(t, 2, "git tag -''d \"$tag\""),
     {"d"}),
    ("步驟 2 補一行 git push origin +${x}refs/（空變數拆 +refs/，左鄰不是字元）",
     lambda t: append_to_block(t, 2, "git push origin +${x}refs/tags/x"),
     {"d"}),
    ('步驟 2 補一行 git p"${x}"ush -f（引號包著空變數；先拿變數才露出空引號）',
     lambda t: append_to_block(t, 2, 'git p"${x}"ush -f origin "refs/tags/$tag"'),
     {"d"}),
    ("步驟 2 補一行 git -C '''' push -f（正規化拿掉整個值；原文照比）",
     lambda t: append_to_block(t, 2, "git -C '''' push -f origin \"refs/tags/$tag\""),
     {"d"}),
    ('步驟 2 補一行 git -C \\"x p"us"h … fetch（豁免面不認引號值；`\\"` 讓引號配對與 shell 不一致）',
     lambda t: append_to_block(t, 2, 'git -C \\"x p"us"h -f origin +refs/heads/x" fetch origin"'),
     {"d"}),
    ("步驟 2 補一行 git ${x}push -f（字首墊空變數）",
     lambda t: append_to_block(t, 2, "git ${x}push -f origin refs/tags/x"),
     {"d"}),
    ("步驟 2 補一行 git${x} push -f（字尾墊空變數）",
     lambda t: append_to_block(t, 2, "git${x} push -f origin refs/tags/x"),
     {"d"}),
    ('步驟 2 補一行 git ""push -f（字首墊空雙引號）',
     lambda t: append_to_block(t, 2, 'git ""push -f origin refs/tags/x'),
     {"d"}),
    ("步驟 2 補一行 git'' push -f（字尾墊空單引號）",
     lambda t: append_to_block(t, 2, "git'' push -f origin refs/tags/x"),
     {"d"}),
    ('步驟 2 補一行 git push origin "${sha}:refs/tags/x"（字內變數為空時即遠端刪除）',
     lambda t: append_to_block(t, 2, 'git push origin "${sha}:refs/tags/x"'),
     {"d"}),
]
...
POSITIVE = [
    ...
    # ── 前置選項的引號值（issue #189 目標 1） ──
    ('步驟 1 補一行 git -C "a b" fetch origin main（合法讀取）',
     lambda t: append_to_block(t, 1, 'git -C "a b" fetch origin main')),
    # ── 展開正規化只動清單三形的指定位置（issue #189 目標 2） ──
    ('步驟 1 補一行 git -C "${d}" fetch（引號裡的變數拿掉後仍是一個空引數）',
     lambda t: append_to_block(
         t, 1, "git -C \"${d}\" fetch origin '+refs/heads/main:refs/remotes/origin/main'")),
    ("步驟 1 補一行 git -C ${d} fetch（單獨成字的變數不動）",
     lambda t: append_to_block(
         t, 1, "git -C ${d} fetch origin '+refs/heads/main:refs/remotes/origin/main'")),
    ("步驟 1 補一行 git --git-dir=${d} fetch（`=` 後的整個值：變數不動）",
     lambda t: append_to_block(
         t, 1, "git --git-dir=${d} fetch origin '+refs/heads/main:refs/remotes/origin/main'")),
    ('步驟 1 補一行 git --git-dir="${d}" fetch（`=` 後的整個值：拿掉變數後的空引號對不動）',
     lambda t: append_to_block(
         t, 1, "git --git-dir=\"${d}\" fetch origin '+refs/heads/main:refs/remotes/origin/main'")),
    ('步驟 2 補一行 git push origin "${sha:?}:refs/tags/x"（帶運算子的展開不在清單內，不動）',
     lambda t: append_to_block(t, 2, 'git push origin "${sha:?}:refs/tags/x"')),
]
