...
10. 核對 issue「派工前讀審」行——依 `L7` 條件應觸發而該行為「未觸發」、空白、所填 URL 的 verdict 不是 `READY` 亦非 `L7` 停損後人的書面裁決者，列為 block。再核 T：`READY`／裁決的 `Issue @` 行所記 T 識別須與 issue 當前「基準」欄的 T 相同，不同者列為 block；本體變更的核對本項暫不做。
