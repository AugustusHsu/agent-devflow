...
10. 核對 issue「派工前讀審」行——依 `L7` 條件應觸發而該行為「未觸發」、空白、所填 URL 的 verdict 不是 `READY` 亦非 `L7` 停損後人的書面裁決、者，列為 block。再核錨：`READY`／裁決的 `Issue @` 行所記 T 識別須與 issue 當前 T 相同；本體 `updatedAt` 晚於錨時，須確認其後變更只在「派工紀錄」「審查處置」兩段（forge 提供本體編輯歷史者以之核；不提供者以協調者派工留言所附的錨時點本體全文比對，未附即 block）。T 不同、或兩段以外有變更者，列為 block。
