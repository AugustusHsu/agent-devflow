...
10. 核對 issue「派工前讀審」行——依 `L7` 條件應觸發而該行為「未觸發」、空白、所填 URL 的 verdict 不是 `READY` 亦非 `L7` 停損後人的書面裁決者，列為 block。再核錨：`READY`／裁決的 `Issue @` 行所記 T 識別須與 issue 當前「基準」欄的 T 相同；本體 `updatedAt` 晚於錨時，以協調者派工留言所附的錨時點本體全文與當前本體比對，差異只得在「派工紀錄」「審查處置」兩段且只得是紀錄行（`L7`）。T 不同、派工留言未附全文、無法比對、或兩段以外有差異者，皆列為 block。
