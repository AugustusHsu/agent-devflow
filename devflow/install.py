...
用法：
    ...
    exit 2：無法執行（目標路徑不對、模板不合規、讀寫失敗）。exit 1／2 時不寫任何檔、
    stdout 全部抑制。
...
- 標記行＝ `line.strip() == "<!-- devflow:begin -->"`（或 end），字面比對，不看 markdown
  結構、不看縮排、不看是否在 code fence 內。行以 \\n 切、decode 後用 str.strip()，
  與 CI 的 text.split("\\n") ＋ str.strip() 同一套語意。
...
def is_marker(line, marker):
    """判準 B：decode 後 str.strip() 整行等於標記。

    用 str.strip() 而非 bytes.strip()：兩者的空白集合不同（str 多 \\x1c-\\x1f、U+00A0 等），
    CI 在 str 上比對，這裡跟它一致。errors="replace" 讓非 UTF-8 的行也能比（U+FFFD 不是
    空白，不會誤判）。
    """
...
def choose_targets(root):
    present = [name for name in ENTRY_FILES if (root / name).exists()]
    if present:
        return present
...
def decide(root, name, template):
    ...
    if begin is None:
        return Decision(name, display, "insert", data, template + b"\n" + data)  # AC-2
    if end is None:
    ...
    return Decision(name, display, "replace", data, data[:start] + template + data[stop:])  # AC-4
...
def write_diff(out, d):
    ...
    name = d.name.encode()
    for line in difflib.diff_bytes(difflib.unified_diff, old, new,
...
def run(target, dry_run):
    ...
    # 原子性：先對集合內全部檔案做決策，任一檔出錯則皆不寫、stdout 全抑制
    decisions, errors = [], []
    for name in targets:
        try:
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        ...
        epilog="模板：%s。exit 0 成功、1 內容碰撞（begin 無 end）、2 無法執行。" % TEMPLATE_PATH)
