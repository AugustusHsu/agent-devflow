...
- `ST6` stage 1 的規格核准與 T 變更：第 2、3 節未生效時，`L1` 所需的 T 可為 `docs/spec/` 草稿（`version: 0.0.0.0`，以 issue 上寫的 commit 為準）或 issue 本體。T 的修訂與本檔的變更皆經 PR，由人在 issue 裁決方向後合入，審查依舊 G（`G2`）；`F2`、`G2` 所指的第 2 節路徑以此代之。修訂後的影響分析依 `S3`、本檔的版本位數依 `V2`，由 orchestrator 自判、審查者核，不因第 2、3 節未生效而免除。`ST2` 起本條失效，改走第 2、3 節。
