...
- `R11` `REQUEST_CHANGES` 的每條阻擋項，在下一輪審查開始前須由協調者在該 PR 留言標一個處置，三值之一：`FIX`（附修正的 commit sha）、`DEFER`（附承接的 issue 號）、`REJECT`（附可重跑的反證：指令與輸出）。未標處置的阻擋項視為未結，不得進下一輪；`REJECT` 的反證由下一輪審查者核對。處置留言的 URL 記入 issue 的派工紀錄。
