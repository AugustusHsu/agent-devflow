...
- `R9` 對照表狀態欄取三值之一。`✅ 可用`：第三者現在照著做能得到值欄宣稱的結果；須附可重跑的驗證方式與受測環境（`R8`、`R10`）。`📝 已宣稱`：有人做過並回報，但沒有可重跑的驗證；須標明自述來源與「未經獨立驗證」，無具名來源者標 `⬜`。`⬜ 未測`：沒做過。`📝` 只是資訊，不足以支持派工決策：不得據以宣告能力可用、不得當作任一條 AC 的證據、不得替代 `✅` 放行任何步驟；升為 `✅` 只能補上驗證方式與受測環境，不得以回報次數或「沒有反例」代替。
- `R10` `✅` 須連同受測環境記載：驗證日期、所用工具與版本、執行身分或權限、目標 repo 或環境，以及其他會改變結果的條件。引用該格前先比對自己的環境與所記載者，有差異的項目重驗後再用，不沿用原狀態。記載的條件變更時（工具升版、權限或設定改變）該格降為 `📝`，原記載留作來源；重驗後才回 `✅`。
