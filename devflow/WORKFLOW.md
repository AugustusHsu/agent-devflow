...
- `L7` 派工前讀審：符合觸發條件的 issue，須先經獨立讀審回 `READY` 才得派工。
  ...
  - **停損**：第二輪仍 `REVISE` → 升人，不自動第三輪；本句優先於上列「處置」的重跑要求。人可 (i) 以書面裁決代 `READY`——本款所稱裁決指人對 `L7` 的各 block 或 T 的處分，留言須具名記載人的指示來源，並含同格式的 `Issue @` 行，未含者不構成 `L7` 的書面裁決；多則裁決時以最新一則含該行者為準；失效條件同 `READY`；或 (ii) 授權再跑一輪；(ii) 的那輪仍 `REVISE` 即回到本款，由人再選。本條生效前的既有裁決依其原記載引用，不受本款限制。
  - **留痕**：issue「派工紀錄」段的「派工前讀審」行，讀審結束即填；進行中可記各輪 URL，**派工當下**該行末項（最後一個 `→` 之後；無 `→` 時為冒號之後整段；不含括號註解）必為三值之一：`READY <URL>`、`人裁決 <URL>`、`未觸發 a／b 皆否`。
