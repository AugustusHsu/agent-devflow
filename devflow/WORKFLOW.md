...
- `R2` 建議審查者（職位，`seats/reviewer.md`）由與 coder 工具（`devflow.yml` 的 `seats.implementer.filler` 值）異廠的工具填；只有一家可用時，用同廠的全新 context。同廠時另建議所用模型與實作位所用者（`seats.implementer.model`）不同；同廠只有一個堪用模型時，全新 context 即滿足本條。這裡的「同廠」看的是當次實際擔任審查的工具與模型，來自 `devflow.yml` 的 `seats.reviewer` 或 `seats.reviewer.fallback` 都一樣。要事先定下同廠改派的工具與模型，可寫在 `devflow.yml` 的 `seats.reviewer.fallback`（選填，`filler`、`model`）。
