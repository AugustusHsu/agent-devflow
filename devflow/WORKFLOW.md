...
- `R9` 對照表狀態欄取三值之一。`✅ 可用`：第三者現在照著做能得到值欄宣稱的結果；須附可重跑的驗證方式與受測環境（`R8`、`R10`）。`📝 已宣稱`：來源分兩種，須標明是哪一種。「自述未驗證」——有人做過並回報，但沒有可重跑的驗證；須標明自述來源與「未經獨立驗證」，無具名來源者標 `⬜`。「先前驗證已失效」——曾標 `✅`，`R10` 記載的條件變更後降級，原驗證不適用於當前條件；須保留原驗證方式與原受測環境。`⬜ 未測`：沒做過。`📝` 只是資訊，不足以支持派工決策：不得據以宣告能力可用、不得當作任一條 AC 的證據、不得替代 `✅` 放行任何步驟。升為 `✅` 的路徑依來源而定：自述未驗證者補上驗證方式與受測環境，先前驗證已失效者在當前條件下重驗；兩者都不得以回報次數或「沒有反例」代替。
