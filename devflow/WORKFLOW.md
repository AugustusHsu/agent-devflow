...
- `V3` normative ＝ bump ≥ `c`；editorial ＝ bump `d`。兩者皆需 issue（`I1` 的分支名由此取 N）、皆走 PR、皆 bump。判斷依語意，不依章節位置或字數。
...
- `L7` 派工前讀審：符合觸發條件的 issue，須先經獨立讀審回 `READY` 才得派工。
  ...
  - **讀審者**：fresh-context；不得由撰寫該 issue 的 session 擔任；建議與協調位工具（`devflow.yml` 的 `seats.coordinator.filler` 值）異廠，協調位由人填時改取 `seats.implementer.filler`，只有一家可用時用同廠的全新 context。執行邊界依 `R12`。
  - **材料**：issue 本體（附 `gh issue view <N> --json updatedAt` 的值）＋全部留言、G、repo 在 base 的 checkout。模板 `templates/issue-review-prompt.md`。
  - **回覆**：`READY` 或 `REVISE`；以 `Issue @ <T 識別> + <本體 updatedAt>`（T 識別＝T 留言 id 或 commit sha）一行寫明所綁的 issue 狀態，並寫明讀審者的工具與 session 識別；`REVISE` 每個 block 四欄 `Blocker`／`Evidence`／`Minimum revision`／`Acceptance check`。`READY` 之後 T 變更，該 `READY` 失效；本體變更的失效判準本條暫不規範。
  ...
  - **停損**：第二輪仍 `REVISE` → 升人，不自動第三輪；本句優先於上列「處置」的重跑要求。人可 (i) 以書面裁決代 `READY`——本款所稱裁決指人對 `L7` 的各 block 或 T 的處分，留言須具名記載人的指示來源（由人本人張貼者，作者欄位即為具名），並含同格式的 `Issue @` 行，未具名或未含者不構成 `L7` 的書面裁決；多則裁決時以最新一則含該行者為準；失效條件同 `READY`；或 (ii) 授權再跑一輪；(ii) 的那輪仍 `REVISE` 即回到本款，由人再選。本條生效前的既有裁決依其原記載引用，不受本款限制。
  - **留痕**：issue「派工紀錄」段的「派工前讀審」行，讀審結束即填；進行中可記各輪 URL，**派工當下**該行末項（最後一個 `→` 之後；無 `→` 時為冒號之後整段；不含括號註解）必為三值之一：`READY <URL>`、`人裁決 <URL>`、`未觸發 a／b 皆否`；辨識三值時，只允許剝除恰好一組、開閉相同、取自 `**`／`__`／`*`／`_` 的外層分隔符（不得巢狀或重複），剝除後須逐字等於三值之一；刪除線、連結、行內程式碼及任何改動 `<URL>` 字元的標記皆不剝除，含之即不屬三值。

## 5. 審查（R）

- `R1` 實作 PR 由 fresh-context 審查者審：拿到 G、T、`base..head`、完整來源、測試指令；不接受作者摘要當證據。同一 session 不得對自己的工作簽正式 verdict。
- `R2` 建議審查者（職位，`seats/reviewer.md`）由與 coder 工具（`devflow.yml` 的 `seats.implementer.filler` 值）異廠的工具填；只有一家可用時，用同廠的全新 context。同廠時另建議所用模型與實作位所用者（`seats.implementer.model`）不同；同廠只有一個堪用模型時，全新 context 即滿足本條。要事先定下同廠改派的工具與模型，可寫在 `devflow.yml` 的 `seats.reviewer.fallback`（選填，`filler`、`model`）。
...
- `R9` 對照表狀態欄取三值之一。`✅ 可用`：第三者現在照著做能得到值欄宣稱的結果；須附第三者現在能執行的驗證方式與受測環境（`R8`、`R10`）。`📝 已宣稱`：有人做過並回報，來源具名；該格既有的驗證方式與受測環境記載不得刪去。依該格有無第三者現在能執行的驗證方式分兩種，須標明是哪一種。「無可執行的驗證方式」——該格沒有第三者現在能執行的驗證方式，不論先前是否驗證過。「驗證未達 `✅`」——該格附有第三者現在能執行的驗證方式，但不滿足 `✅` 的成立條件（例如未涵蓋當前宣稱、重跑結果不符、受測環境（`R10`）未附齊或與當前條件不符、須等待特定外部狀態（如配額耗盡、特定時段）才能重現宣稱結果；不以此為限）。「現在能執行」指第三者不依賴他人協助即可照著做；是否能在任意時點重現宣稱結果不影響歸類，只影響能否升 `✅`。一格兼有兩者時，比照 `R7` 逐項標明是哪一種。`⬜ 未測`：沒做過，或做過的說法沒有具名來源。`📝` 只是資訊，其效力限制以下列三項為限：不得據以宣告能力可用、不得當作任一條 AC 的證據、不得替代 `✅` 放行任何步驟。升為 `✅` 須使 `✅` 的成立條件全部成立；不得以回報次數、「沒有反例」或補齊部分條件代替。對照表得分為通用節與本機節。分節時先歸節、後判狀態。歸節只看值欄宣稱的對象，不看狀態欄：宣稱對象是具名的環境 instance——本機安裝的工具、帳號或組織、本 repo 的設定與資源——歸本機節；宣稱對象是可重用的工具能力或程序、repo 與環境只是可替換的參數者，歸通用節。值欄未表明宣稱對象時，先改寫值欄使其表明，不得直接歸節。一格兼含兩種宣稱時，拆成各自獨立的原子事實分別歸節，不把同一事實抄進兩節（`I5`、`R7`）。歸節後，各節的 `✅ 可用` 依下列定義，`📝`、`⬜` 及三值間的升降在兩節相同。通用節的 `✅ 可用`：第三者在任何專案照該格所附驗證方式做，能得到值欄宣稱的結果。本機節的 `✅ 可用`：第三者在本 repo 的當下環境照該格所附驗證方式做，能得到值欄宣稱的結果。兩節的 `✅` 皆須附第三者現在能執行的驗證方式與受測環境（`R8`、`R10`）；`R10` 的記載、比對與降級對兩節皆適用。未分節的表，`✅ 可用` 依本條前述定義。
- `R10` `✅` 須連同受測環境記載：**首次驗證與最近確認的日期與工具版本**（同一次即寫一次）、執行身分或權限、目標 repo 或環境，以及其他會改變結果的條件。引用者與該格的關係：引用前先比對自己的環境與所記載者，有差異的項目自己重驗後再用；**比對出的差異只約束引用者，不改變該格的狀態**。該格的狀態：「最近確認」指在該格記載的路徑上實跑一次且結果與值欄相符，並附該次的 forge 紀錄（不限新版本；同版本重跑亦算）；**最近確認距今超過 30 天**、或權限／設定變更使記載條件不再成立時，該格依 `R9` 降為 `📝`。工具升版本身不改變該格狀態。
- `R11` `REQUEST_CHANGES` 的每條阻擋項，在下一輪審查開始前須由協調者在該 PR 留言標一個處置，三值之一：`FIX`（附修正的 commit sha）、`DEFER`（附承接的 issue 號；只適用於不屬本 PR 對應 AC 的阻擋項——AC 未達者依 `F1` 在同一 issue 修正，不得以 `DEFER` 放行）、`REJECT`（附可重跑的反證：指令與輸出）。未標處置的阻擋項視為未結，不得進下一輪；`REJECT` 的反證由下一輪審查者核對。處置留言的 URL 記入 issue 的「審查處置」段（`templates/issue.md`）。
- `R12` `L7` 讀審與 `R1` 審查在丟棄式 checkout 內執行，不交主 checkout 或任務 worktree；工具須設為不得自行升權，可寫根收斂到該 checkout 與本輪專用暫存目錄兩者，`/tmp` 本身不在可寫根內。可寫根內的 `.git` 可能被工具保護為唯讀（防竄改 hook 與沙箱設定），該保護不視為可寫根收斂失敗；需要 commit 才能取得的量測，在本輪專用暫存目錄內以 `git clone --shared` 重建同 base 的 checkout 後於該處進行，隨暫存目錄丟棄。讀審者與審查者不得對被審 checkout 的 `.git` 寫、不得 push、不得對 forge 寫；verdict 由協調者貼出。派工的 prompt 須寫明本輪沙箱下哪些失敗是預期的——可寫根外的寫入、網路存取、被審 checkout 的 `.git` 寫入——以及暫存一律用本輪專用暫存目錄。協調者於 verdict 留言記載本輪的沙箱設定與可寫根實際值，並於該輪結束前核三項：checkout 的 HEAD 等於被審 sha、該輪工具日誌顯示的核准政策為「不得自行升權」、該 checkout 的 `git diff` 已另存；工作樹可為 dirty，隨 checkout 丟棄。工具設不起可寫根收斂時，該輪改唯讀執行並於派工 prompt 與 verdict 留言雙方註明，「不得自行升權」不因此豁免；連「不得自行升權」都設不起來的工具，不得擔任該輪，須改派。
