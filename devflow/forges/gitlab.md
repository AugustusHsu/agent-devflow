# forge: gitlab

衍生值對照表，**只有通用節**（值欄宣稱可重用的工具能力或程序，repo 與環境只是可替換的參數）。本機節——具名的環境 instance——住 `devflow.local/<同路徑>`，不隨 kit 安裝。分節與狀態欄依 `R9`。
本 repo 沒有 GitLab 實例；預定在導入 AI_Server_SuperOD 時實測。

## 通用

| 面向 | 職位 | 值 | 狀態 |
|---|---|---|---|
| 審查載體 | — | Merge Request | ⬜ 未測 |
| CLI | — | `glab` 或 `curl` REST v4 | ⬜ 未測（依 `R9` 自原「CLI」一格拆出；原值欄的「本機未安裝」是本機 instance 事實，見本機節「CLI（本機 instance）」格） |
| issue_ref | — | 專案內 `iid`，非全域 `id` | ⬜ 未測 |
| 建 issue／留言／label | implementer／coordinator／approver | `POST /projects/:id/issues`、`/issues/:iid/notes`、`PUT …?add_labels=`（增量；`labels=` 是全量，不用） | ⬜ 未測 |
| 標準 label | — | 導入的 GitLab 專案須建的 label：`spec`、`process`、`bypass`、`blocked` | ⬜ 未測（值欄依 `R9` 改寫以表明宣稱對象：本 repo 無 GitLab 實例，本格只能是導入專案應採的設定，不是任一 instance 的現況） |
| 從 issue 開分支 | coordinator | `POST /repository/branches?branch=<iid>-<slug>&ref=main` | ⬜ 未測 |
| 分支保護 | — | 導入的 GitLab 專案的 main 應設 protected：Allowed to push＝Maintainers、force push 關；Merge method＝Merge commit；Squash＝Do not allow；Delete source branch by default 開 | ⬜ 未測（值欄依 `R9` 改寫以表明宣稱對象：本 repo 無 GitLab 實例，本格只能是導入專案應採的設定，不是任一 instance 的現況） |
| tag 保護 | — | 導入的 GitLab 專案應設 protected tags，Allowed to create＝Maintainers | ⬜ 未測（值欄依 `R9` 改寫以表明宣稱對象：本 repo 無 GitLab 實例，本格只能是導入專案應採的設定，不是任一 instance 的現況） |
| 開 MR | implementer／coordinator（代行） | `POST /merge_requests` | ⬜ 未測 |
| 審查證據（`R3`／`R5`） | coordinator | MR note；approval rules 在 Free 可能 403 → verdict 寫在 note 內並含 head sha | ⬜ 未測 |
| 合併（`I3`） | approver／coordinator（`M2`） | `PUT /merge_requests/:iid/merge` | ⬜ 未測 |
| 已合併訊號（`C1`） | coordinator | `GET /merge_requests/:iid`（`state=merged`、`merge_commit_sha`）＋ `git merge-base --is-ancestor` | ⬜ 未測 |
| 合併後刪分支（`C1`） | coordinator | `should_remove_source_branch` 可能不生效 → `git ls-remote --heads` 驗，多的手動刪 | ⬜ 未測 |
| CI 位置 | — | `.gitlab-ci.yml` | ⬜ 未測 |
| 人類站（`D3`） | — | GitLab Pages（`pages` job 產 `public/`；自架實例需管理員啟用） | ⬜ 未測 |
| 故障退路（`F3`） | coordinator | MR 單筆端點回 500 時合併可能已成功：先讀清單端點與目標分支 sha，再決定 | 📝 已宣稱（`R9` 子類「驗證未達 `✅`」：驗證方式＝依值欄所述在 MR 單筆端點回 500 時照做，第三者不依賴他人協助即可照著做；不達 `✅` 的理由：500 須等待特定外部狀態才能重現，第三者無法按需觸發。來源具名：文件記錄（SuperOD 2026-09-03）；原狀態欄寫「文件記錄（SuperOD 2026-09-03）」，非 `R9` 三值，依 `R9` 定義改標） |

## 已知限制（文件推導，待實測）

- `done` 與 `cancelled` 在平台層同形，只能靠 label 區分。
- 自架實例的 MR 頁面與單筆 API 曾回 500；導入前重新確認當下能力。
- runner 上的 `origin` 可能指向 GitLab 而本機 `origin` 指向鏡像；canonical remote 必須明確記在專案綁定，不從 remote 名稱推導。
