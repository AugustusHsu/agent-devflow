...
| 合併後刪分支（`C1`）——平台自動 | repo 層 `delete_branch_on_merge=true`，合併時自動刪除來源分支；`git ls-remote --heads origin` 驗證 | ✅ 實測 2026-09-09（`gh api repos/AugustusHsu/agent-devflow` 讀回 `delete_branch_on_merge=true`；PR #2 合併後未另下刪除指令，timeline 於合併後一秒內記到 `head_ref_deleted`，`git ls-remote --heads origin` 只剩 `refs/heads/main`） |
