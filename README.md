...
devflow/                  kit 本體：安裝時整個目錄鏡像到消費者專案
...
.github/workflows/        devflow-checks.yml：CI 入口（check 列入 branch protection），呼叫 scripts/devflow_checks.py；docs.yml：人類站發布
.mkdocs/                  人類站設定：mkdocs.yml、requirements.txt、hooks/（表格 code span 的跳脫對齊 GFM）
scripts/                  devflow_checks.py：CI 檢查器本體。每項檢查各有關卡開關（`GATES`）：關卡失敗擋合併，其餘建議只寫進 log；哪幾項是關卡見該檔檔頭
tests/                    smoke_devflow_checks.py：檢查器的正反案例；install/harness.py：安裝器對兩份規格 AC 的驗收測試
.gitignore                只排除本機工具生成物，不排除 devflow.local/
```
...
「證據」欄只記能用 `gh`／`git` 讀回的識別碼（PR 號、merge commit、tag、run 號），不寫日期敘述。
